...
shorttitle        : "Post-publication critique"

author:
  - name: 'Tom E. Hardwicke'
    affiliation: '1'
    corresponding: yes
    email: 'hardwickete@gmail.com'
...
keywords          : "peer review, post-publication critique, letter to the editor, meta-research, journal policy, self-correction, scientific criticism"
...
# Introduction
Poor quality research frequently survives peer review and permeates through to the academic literature (Altman, 1994; Hardwicke, Serghiou, et al., 2020; Ioannidis, 2005; Nosek et al., 2022; Van Calster et al., 2021). This highlights the importance of ongoing critical scrutiny of published research to identify errors, limitations, or alternative interpretations that were not adequately addressed during pre-publication peer review. Such critiques may help research consumers to make more informed judgements about the utility and validity of published scientific claims (Altman, 2005; Bastian, 2014).

Journals exert considerable control over criticism of prior research submitted in the form of letters, commentaries, or online comments (Altman, 2002; Winker, 2015). We refer to these collectively as ‘post-publication critique’ (see Supplementary Information K for an operational definition). Currently, there is limited empirical data available to systematically evaluate how journals handle this important aspect of scientific self-correction. Prior studies of post-publication critique were narrow in scope, mainly focused on medical journals, and are now outdated (Altman, 2002; Kastner et al., 2013; Von Elm et al., 2009; Winker, 2013). In the present research, we sought to provide a systematic, cross-disciplinary, and more contemporary assessment of journal policies (Study One) and practice (Study Two) related to post-publication critique at 330 top-ranked journals operating in 22 scientific disciplines (Supplementary Information M provides a schematic illustration of the two studies). Our goal was to generate empirical evidence to inform debates about how scientific critique should be optimally handled by academic journals. 
...
The sample consisted of 15 academic journals, top-ranked by 2017 Journal Impact Factor operating in each of 22 scientific disciplines (N = 330 journals). This represents the entire population of interest. Journal Impact Factors were identified using Clarivate Journal Citation Reports (https://jcr.clarivate.com). Scientific disciplines were defined by Clarivate Essential Science Indicators: https://perma.cc/MD4V-A5X5; Supplementary Information L). We did not include journals that only published reviews. The sample size was chosen based on a precision analysis which is documented in our preregistered protocol (https://osf.io/hjvnw/).

### Design

The study had a cross-sectional design. The measured variables (see Supplementary Information B for details) were the name and description of any options for post-publication critique offered by each journal (e.g., letters), limits imposed on post-publication critique in terms of length (e.g., number of words), time-to-submit (e.g., weeks since publication of the target article), or number of references, and whether post-publication critiques are sent for independent external peer-review (reviews solicited from individuals who were not members of the editorial team or authors of the target article). We also obtained 2017 Journal Impact Factors and identified whether journals were members of the Committee on Publication Ethics (COPE).

### Procedure

1.	Between November, 2019, and January, 2020, TEH and VEK identified and preserved the ‘article types’ section of the author submission guidelines on each journal’s website (Supplementary Information C).
...
5.	If an article type seemed like it might be post-publication critique, but the description was insufficient to judge, coders checked several published articles of this type to determine whether any met our operational definition of post-publication critique (Supplementary Information K).

6.	Because journals used various naming conventions for similar types of post-publication critique and various units to specify limits (e.g., characters, words, or pages for length limits), we harmonized names to four types (‘letters [to the editor]’, ‘commentaries’, ‘web comments’, and ‘other’) and transformed units to words (length limits) and weeks (time-to-submit limits). Letters and commentaries were types of articles submitted through the regular manuscript submission system, whereas web comments were submitted via an online interface on the target article webpage. For details, see Supplementary Information D.
...
Across all 22 scientific disciplines (n = 330 journals), the median 2017 Journal Impact Factor was `r allFieldChar %>% pull(Median_jif)` (range `r allFieldChar %>% pull(Min_jif)` - `r allFieldChar %>% pull(Max_jif)`). `r allFieldChar %>% mutate(COPE_report = paste0(COPE_Signatories, ' (', round((COPE_Signatories/330)*100,0),'%)')) %>% pull(COPE_report)` journals were members of COPE. Characteristics for each scientific discipline are available in Supplementary Table E1.
...
Figure \@ref(fig:fig-policy-freq) shows the number of journals offering any form of post-publication critique, and specific types of post-publication critique, across 22 scientific disciplines (for equivalent tabular data, see Supplementary Table F1). Of `r journalsOfferingAnyPPPR$n` journals that offered post-publication critique, `r pppr_cope` (`r round(pppr_cope/journalsOfferingAnyPPPR$n,2)*100`%) were members of COPE. Of `r journalsOfferingAnyPPPR$N - journalsOfferingAnyPPPR$n` journals that did not offer post-publication critique, `r no_pppr_cope` (`r round(no_pppr_cope/(journalsOfferingAnyPPPR$N - journalsOfferingAnyPPPR$n),2)*100`%) were members of COPE. 

Journals offering post-publication critique were most common in `r str_to_title(high_field_PPPR$field)` (n = `r high_field_PPPR$n`, `r high_field_PPPR$percent`%) and least common in `r str_to_title(low_field_PPPR$field)` (n = `r low_field_PPPR$n`, `r low_field_PPPR$percent`%). Overall, `r journalsOfferingOnePPPR` journals offered one option for post-publication critique, `r journalsOfferingTwoPPPR` journals offered two options, and `r journalsOfferingThreePPPR` journals offered three options, equating to a total of `r d_policy %>% filter(!is.na(PPPR_name)) %>% nrow()` individual post-publication critique options across journals.
...
```
After post-publication critique names were harmonized into four types, there were `r letters_n` journals offering letters, `r commentaries_n` journals offering commentaries, and `r web_comments_n` journals offering web comments. `r other_n` journals offered other miscellaneous types of post-publication critique such as ‘Forum papers’ and ‘Update articles’. A complete list of journals and their post-publication critique options is available in  Supplementary Table G1.
...
PPPR_category_plotter <- function(d){
   ggplot(d, aes(y = PPPR_category, x = n, label = ifelse(var_type == "Any PPC",paste0(round((n/15)*100,0),'%'),''), colour = var_type)) +
...
PPPR_category_plotter_allFields <- function(d){
   ggplot(d, aes(y = PPPR_category, x = n, label = 
     ifelse(var_type == "Any PPC",paste0(round((n/330)*100,0),'%'),''), 
     colour = var_type)) +
    geom_segment(aes(x=0, xend=n, y=PPPR_category, yend=PPPR_category)) +
    geom_point(aes(size = var_type)) +
    geom_text(color="black", size = 2.5) +
...
thisData <- d_policy %>% 
  ...
  mutate(
    PPPR_category= ifelse(PPPR_category == "Any PPPR", "Any PPC", PPPR_category),
    PPPR_category = factor(PPPR_category, levels = c("Other", "Web comments", "Commentaries", "Letters", "Any PPC")),
    var_type = ifelse(PPPR_category == "Any PPC", "Any PPC", "Other"),
...
```{r fig-policy-freq, fig.width = 7.5, fig.height = 10, fig.path = 'figs/', fig.cap = 'Number and percentage of journals offering any (purple) and specific (green) options for post-publication critique (PPC) in all scientific disciplines (top-left panel, n = 330 journals) and in individual scientific disciplines (other panels, n = 15 journals). Note that individual journals can offer multiple post-publication critique options.'}
megaPlot
```
...
limitCategoriesPlot <- function(d, thisTitle){
    ...
    ylab(expression(paste('PPCs (',italic('n'),')'))) +
...
2.	For each article, coders ascertained whether it (1) needed to be excluded; (2) was itself an example of post-publication critique; or (3) was linked to post-publication critique. When we encountered multiple post-publication critiques that were part of the same back-and-forth exchange between target article authors and post-publication critique authors, these were counted as a single instance of post-publication critique. Coders followed an instruction sheet (https://osf.io/aejx4/), which reminded them of the exclusion criteria and operational definition of post-publication critique (Supplementary Information K), and entered data directly into a Google Sheet. Coders were encouraged to discuss any ambiguous cases with TEH and all positive post-publication critique classifications were independently verified by TEH.

3.	When journals relied on third-party services (e.g., Elsevier’s ScienceDirect) to distribute their articles we only used these websites if the journal did not also distribute their articles through their own dedicated website (as we noticed that links to post-publication critique were sometimes displayed on journal websites and not on third-party websites).

4.	Coders worked sequentially through each journal’s articles until they had examined ten that were eligible (i.e., they were not excluded or classified as themselves being post-publication critique).

5.	For the assessment of post-publication critique features, one author (TEH) extracted information (see Supplementary Table I) from all linked post-publication critiques identified by the primary estimate method.
...
```
In total, we considered `r d_freq_checked` articles for inclusion before we reached our target of 2066 eligible research articles. `r d_freq_excluded` articles were excluded because they did not contain research (n = `r d_freq_notResearch`) or could not be found/accessed (n = `r d_freq_noAccess`). An additional `r d_freq_isPPPR` articles were classified as being themselves examples of post-publication critique and excluded from our primary prevalence estimate. In total, `r linkedPPPR_n` of the 2066 research articles were linked to at least one post-publication critique. Our primary post-publication critique prevalence estimate was therefore `r prev_est_primary`. These articles were published in `r d_freq %>% filter(linkedPPPR == T) %>% count(journal) %>% nrow()` individual journals (Supplementary Table J2)).

We also computed a secondary prevalence estimate based on the proportion of articles that were themselves post-publication critique  amongst the first 10 eligible articles assessed at each journal (first 6 articles in the case of Wildlife Monographs; N = 2066 articles). We only examined the first 10 eligible articles in order to align the denominator of the primary and secondary estimates. This meant that articles classified as being themselves examples of post-publication critique only contributed if they were found within the first 10 eligible articles, and thus, we did not include all of the 173 post-publication critiques found among the 3030 articles mentioned above. `r isPPPR_n` out of 2066 articles were classified as post-publication critique, yielding a secondary prevalence estimate of `r prev_est_secondary`. This is equivalent to `r round(ratio_secondary,1)` post-publication critiques for every 100 research articles. These articles were published in `r d_freq_secondary %>% group_by(journal) %>% slice_head(n = 10) %>% filter(isPPPR == T) %>% count(journal) %>% nrow()` individual journals (Supplementary Table \@ref(tab:tab-practice-prev-secondary-journals)). 
...
```{r}
linkedPPPR_med <- d_freq_primary %>% count(linkedPPPR, ESI_field) %>% filter(linkedPPPR==T, ESI_field == "CLINICAL MEDICINE") %>% pull(n)

isPPPR_med <- d_freq_secondary %>% count(isPPPR, ESI_field) %>% filter(isPPPR==T, ESI_field == "CLINICAL MEDICINE") %>% pull(n)
```
Prevalence estimates varied substantially across scientific disciplines (Figure \@ref(fig:fig-practice-prev); for equivalent tabular data see Supplementary Table J1). The number of disciplines which had zero instances of post-publication critique was `r primary_zero` according to our primary prevalence estimate and `r secondary_zero` according to our secondary prevalence estimate. The discipline with the most post-publication critique was Clinical Medicine with `r linkedPPPR_med` (`r primary_max$primary`%) of 150 research articles being linked to post-publication critique and `r isPPPR_med` (`r secondary_max$secondary`%) of 150 assessed articles being themselves instances of post-publication critique.

### Features of post-publication critique in practice
...
We conducted a closer examination of the post-publication critique linked to the `r linkedPPPR_n` articles identified above for our primary prevalence estimate. These `r linkedPPPR_n` articles were each linked to either one (*n* = `r d_assess_articles %>% filter(ppprs==1) %>% pull(n)`), two (*n* = `r d_assess_articles %>% filter(ppprs==2) %>% pull(n)`), three (*n* = `r d_assess_articles %>% filter(ppprs==3) %>% pull(n)`), or four post-publication critiques (*n* = `r d_assess_articles %>% filter(ppprs==4) %>% pull(n)`); a total of `r d_assess %>% count(article_id) %>% pull(n) %>% sum()` post-publication critiques. Various features of these post-publication critiques are shown in Tables 3, 4, and 5 (Table 3 in the actual manuscript) and features of target article author responses to these post-publication critiques are shown in Table 6 (Table 4 in the actual manuscript).
...
) %>% kable(caption = "Table 3A: Features of 58 assessed post-publication critiques.") %>% kable_styling(latex_options=c("hold_position")) %>% column_spec(1, width = "25em") %>% pack_rows("PPPR type", 1, 3) %>% pack_rows("Journals", 4, 13) %>% pack_rows("Domains", 14, 20) %>% pack_rows("Open access to target article and PPPR", 21, 24) %>% pack_rows("PPPR author anonymity", 25, 26) %>% pack_rows("PPPR conflict of interest (COI)", 27, 29) %>% pack_rows("Type of issues raised in PPPR", 30, 35) %>% pack_rows("Data availability in target article", 36, 39) %>% pack_rows("Analyses/data included in PPPR", 40, 43)
```

```{r}
d_assess_words %>% kable(caption = "In manuscript this is part of Table 3. Features of 58 assessed post-publication critiques. Length of post-publication critique (words).", booktabs = T) %>% kable_styling(latex_options=c("hold_position")) 
d_assess_pub_time %>% kable(caption = "In manuscript this is part of Table 3. Features of 58 assessed post-publication critiques. Time of post-publication critique publication relative to target article publication (days)", booktabs = T) %>% kable_styling(latex_options=c("hold_position")) 
```

```{r}
## AUTHOR RESPONSE TO post-publication critique
...
bind_rows(
  ...
  kable(caption = 'Features of target article author response to 58 assessed post-publication critiques.', booktabs = T) %>%
  kable_styling(latex_options=c("hold_position")) %>%
  pack_rows("Did the post-publication critique prompt publication of a correction?", 1, 2) %>%
  pack_rows("Did the post-publication critique prompt publication of an author reply?", 3, 4) %>%
...
```{r warning=F}
# table for Peer Review Congress abstract

# summarise number of journals offering each post-publication critique type in each discipline
sum_domain_type <- d_policy %>%
  count(domain, type = PPPR_name_harmonized, .drop = F) %>%
  filter(type %notin% c('Not categorised','Other'))

# summarise number of journals offering any post-publication critique type in each discipline
sum_domain_any_type <- d_policy %>%
  filter(PPPR_type == 'A') %>% # only need the first entry for each journal
  count(domain, anyPPPR) %>%
  group_by(domain) %>%
  mutate(N = sum(n)) %>%
  filter(anyPPPR == T) %>%
  select(-anyPPPR) %>%
  mutate(type = 'Any PPPR')

sum_domain <- full_join(sum_domain_type, sum_domain_any_type %>% select(-N)) %>%
  right_join(sum_domain_any_type %>% select(domain, N), by = 'domain') 

sum_overall <- sum_domain %>%
    group_by(type) %>%
    summarise(n = sum(n),
           N = sum(N)) %>%
    mutate(domain = 'All')
  
sum_combined <- bind_rows(sum_domain, sum_overall) %>%
  mutate(
    domain = factor(domain, 
                    levels = c('All', 
                               ...
                               'FORMAL SCIENCES',
                               'SOCIAL SCIENCES')),
    values = paste0(n,' (',round(n/N*100,0),'%)')) %>%  # add percentages
  select(-n) %>%
  rename('Features' = type)
  

# now do length and time limits
df_limits <- d_policy %>% 
  filter(!is.na(PPPR_name)) %>% 
  mutate(hasLengthLimit = ifelse(wordLimits %in% c('NOT STATED', 'NO'), F, T),
         hasTimeLimit = ifelse(timeLimits %in% c('NOT STATED', 'NO'), F, T),
         ...
         hasTimeLimit_quant = ifelse(timeLimits %in% c('NOT STATED', 'NO', 'Qualitative limit'), F, T),
         wordLimits = ifelse(
      wordLimits %in% c('Qualitative limit','NOT STATED', 'NO'), NA, as.numeric(wordLimits)),
    timeLimits = ifelse(
      timeLimits %in% c('Qualitative limit','NOT STATED', 'NO'), NA, as.numeric(timeLimits))) 

df_limits_domain <- df_limits %>%
  group_by(domain) %>%
  summarise(
    across(c(hasLengthLimit, hasLengthLimit_quant, hasTimeLimit, hasTimeLimit_quant), sum),
    across(c(wordLimits, timeLimits), 
           list(median = median,
               IQR = ~ paste0(quantile(.x,na.rm=T)[2],'-',quantile(.x,na.rm=T)[4])), na.rm = T)) 

df_limits_overall <- df_limits %>%
  summarise(
    across(c(hasLengthLimit, hasLengthLimit_quant, hasTimeLimit, hasTimeLimit_quant), sum),
    across(c(wordLimits, timeLimits), 
           list(median = median,
...
df_limits_combined <- bind_rows(df_limits_domain, df_limits_overall) %>%
  mutate(lengthLimits = paste0('Any: ', hasLengthLimit,' Quant: ', hasLengthLimit_quant),
         lengthQuant = paste0('Md = ', wordLimits_median, ' IQR: ', wordLimits_IQR),
         timeLimits = paste0('Any: ', hasTimeLimit,' Quant: ', hasTimeLimit_quant),
         timeQuant = paste0('Md = ', timeLimits_median, ' IQR: ', timeLimits_IQR)) %>%
  select(domain, lengthLimits, lengthQuant, timeLimits, timeQuant) %>% 
  pivot_longer(cols = lengthLimits:timeQuant, names_to = 'Features', values_to = 'values')
  
# post-publication critique prevalence by discipline
prevalence_domain <- d_freq_primary %>% 
  mutate(linkedPPPR = factor(linkedPPPR)) %>% 
  count(domain, linkedPPPR, .drop = F) %>% 
  group_by(domain) %>% 
  mutate(N = sum(n)) %>% 
  filter(linkedPPPR == 'TRUE') %>%
  adorn_totals(where = 'row', name = 'All', "n", "N") %>%
  select(-linkedPPPR) %>%
  rowwise() %>%
  mutate(
    percent = round(n/N*100,1),
    ci_lwr = round(prop.test(n,N)$conf.int*100,1)[1],
    ci_upr = round(prop.test(n,N)$conf.int*100,1)[2],
    values = paste0(n,'/',N,', ',percent,'% (',ci_lwr,'-',ci_upr,'%)')
  ) %>%
  select(domain, values) %>%
  mutate('Features' = 'PPPR Prevalence')

# combine everything to make final table
final_table <-bind_rows(sum_combined, prevalence_domain, df_limits_combined) %>%
  select(-N) %>%
  right_join(sum_domain_any_type %>% ungroup() %>% select(domain, N) %>% add_row(domain = 'All', N = 330), by = 'domain') %>%
  pivot_wider(id_cols = Features,
              names_from = c(domain, N),
              values_from = c(values),
              names_glue = "{domain} (N={N})") %>%
  relocate('Features', 'All (N=330)', 'HEALTH & LIFE SCIENCES (N=165)', 'MULTIDISCIPLINARY (N=15)', 'PHYSICAL SCIENCES (N=90)', 'FORMAL SCIENCES (N=30)', 'SOCIAL SCIENCES (N=30)')


# d_policy %>%
#   mutate(limits = timeLimits) %>% # use either wordLimits or timeLimits
#   filter(!is.na(PPPR_name), limits %notin% c('Qualitative limit','NOT STATED', 'NO')) %>%
#   mutate(limits = as.numeric(limits)) %>%
#   group_by(field) %>%
#   summarise(
#     md = median(limits),
#     iqr = paste0(quantile(limits,na.rm=T)[2],'-',quantile(limits,na.rm=T)[4]),
#     min = min(limits),
#     max = max(limits)
#   )
```
...
3.	Clearly state whether post-publication critique will be independently peer reviewed. Recognise that the authors of the target article may provide useful feedback, but cannot be considered neutral.
4.	Facilitate expedient handling of post-publication critique submissions to ensure timely dissemination of relevant critique to research consumers.
5.	Foster a culture of critique. Actively encourage and highlight post-publication critique to the journal’s readership, for example, via editorials.
6.	Enhance access to and discoverability of post-publication critique: (a) Tag post-publication critique with appropriate meta-data so they can be indexed in third-party databases, websites and referencing software; (b) display prominent links to post-publication critique alongside target articles; (c) make post-publication critique open access.
7.	Remove strict length, time-to-submit, and reference limits. Judge post-publication critique on a case-by-case basis and promote concise writing via editorial feedback.
...
9.	Adopt a two-tier post-publication critique system. Tier One involves rapid publication of lightly moderated contributions on the journal’s website (i.e., web comments). Tier Two curates the most informative Tier One contributions and converts them to formal articles (letters) that become a permanent part of the scientific record and provide appropriate academic credit to their authors. For an example two-tier system, see BMJ Rapid Responses (Delamothe & Smith, 2002).
10.	Improve transparency and accountability by hiring an independent editor responsible for handling post-publication critique (Schriger & Altman, 2010). Publish all editorial decisions related to post-publication critique, including the number submitted, rejected, and published.
...
Committee on Publication Ethics. (2019). Core Practices. COPE: Committee on Publication Ethics. https://web.archive.org/web/20191122170830/https://publicationethics.org/core-practices
...
Gotzsche, P. C., Delamothe, T., Godlee, F., & Lundh, A. (2010). Adequacy of authors’ replies to criticism raised in electronic letters to the editor: Cohort study. BMJ, 341, c3926–c3926. https://doi.org/10.1136/bmj.c3926
...
Kastner, M., Menon, A., Straus, S. E., & Laupacis, A. (2013). What do letters to the editor publish about randomized controlled trials? A cross-sectional study. BMC Research Notes, 6:414. https://doi.org/10.1186/1756-0500-6-414
