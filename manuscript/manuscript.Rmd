---
title             : "Avenues for critical discourse at influential scientific journals: A cross-disciplinary assessment of post-publication peer review policies and practice"
...
pppr_types_tab <- pppr_types %>%
  ...
  mutate(
    ...
    time_md_iqr = paste0(round(timeLimits_median,0),' (',round(timeLimits_IQR,0),')'),
...
```{r fig-practice-prev, fig.width = 7.5, fig.height = 10, fig.path = 'figs/', fig.cap = 'Primary and secondary prevalence estimates for the use of PPPR in practice across domains and overall. On-graph text labels are not shown for domains with zero prevalence. Error bars represent 95% confidence intervals.'}
field_prev %>%
  pivot_longer(cols = c(primary, secondary), names_to = 'estimate_type', values_to = 'prevalence') %>%
  mutate(estimate_type = factor(estimate_type, levels = c('primary','secondary'))) %>%
  ggplot(aes(x = estimate_type, y = prevalence, colour = str_wrap(field_with_abbr,30))) +
  geom_beeswarm(size = 3, alpha = .5, side = 1L) +
  geom_errorbar(data = field_prev_all, aes(ymin=ci_lwr, ymax=ci_upr), width = .05, colour = 'black', position = position_nudge(x = 0.07)) +
  geom_point(data = field_prev_all, size = 5, shape = 23, aes(fill = 'OVERALL'), colour = 'black', position = position_nudge(x = 0.07)) +
  scale_fill_manual(values = 'white') +
  scale_x_discrete(expand=expansion(mult = 0, add = c(0.5,0.3))) +
  geom_text_repel(
    aes(label = ifelse(prevalence > 0,as.character(field_abbr),''),
        segment.square = T,
        segment.inflect = T), #
    #point.padding = 0.2,
    ...
    hjust        = 0,
    ylim = c(0,Inf),
    segment.size = 0.5,# size of connector lines
  ...
  ylab('prevalence (%)') +
  xlab('prevalence estimate type') +
  guides(fill = guide_legend(nrow = 1, order = 1, title.position = 'top', title = "Scientific domains")) +
  guides(colour = guide_legend(nrow = 11, order = 2,  title.position = 'top', title = ''))
```
